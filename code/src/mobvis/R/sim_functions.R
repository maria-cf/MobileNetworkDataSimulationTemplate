...
sim_get_trajectory_data <- function(sim, device = NULL) {
    ...
    if (is.null(device)) {
        device <- unique(x$PhoneId)
    } else {
        if (!(all(device %in% x$PhoneId))) stop("Device ID(s) not found in ", f)
    }
    x %>%
...
#' @name sim_get_trajectory_data_TA
#' @rdname sim_data
#' @export
sim_get_trajectory_data_TA <- function(sim, device = NULL) {
    f <- file.path(sim$networkEvents_dir, paste0("AntennaInfo_MNO_", sim$mno, ".csv"))

    x <- tidyr::as_tibble(fread(f, colClasses = c('integer', rep('character', 5), rep('numeric', 2), 'character')))
    ...
    x %>%
        dplyr::rename(cell = 2, event = 3, dev = 4, TA = 6, tile = 9) %>%
        dplyr::select(t, cell, TA, dev, x, y) %>%
        dplyr::filter(dev == device) %>%
        st_as_sf(coords = c("x", "y"), crs = sim$crs)
}
